...
- **多 AI 全屏对比** — 点击侧栏工具栏 `▦`，一次输入同时发送给多个 AI，并排查看各原站回答；支持统一缩放并记住比例
- **选择页面元素** — 像 DevTools 一样选取当前页面元素，自动提取文本到扩展自己的阅读器，并填充到当前侧栏应用（Harness 或 AI 站点）的输入框
- **页面元素阅读器** — 选中元素后弹出浮动面板，可展开查看或复制文本；填充输入框失败时面板会说明原因
...
- **按页面隔离侧栏** — 仿照 Codex 扩展，按 Chrome `tabId` 保存每个页面的应用、Harness 对话路由、阅读器和已打开页面的缩放状态；新建标签页继承最近一次应用和缩放偏好
- **独立 / 统一侧栏可切换** — 设置页「侧边栏行为」开关：默认每个标签页独立侧栏；关闭后所有标签页共享同一个侧栏，新标签页延续当前内容，切换标签页时侧栏保持不变（选择立即生效并记住）
- **默认只显示聊天站点** — 新安装只显示 DeepSeek、智谱、千问、Kimi、ChatGPT、Gemini；DeepSeek Harness 与有道词典默认隐藏，可在设置页勾选显示
...
- **后台标签页释放资源** — 侧栏隐藏 30 秒后释放该文档里的全部 iframe，切回来时自动重建，不再按标签页累积内存
...
- **简洁工具栏** — 深色主题，不干扰对话体验；操作图标统一为同一套内联 SVG（相同尺寸、线宽和颜色），右侧操作区为紧凑尺寸（按钮 15px、图标 10.5px、百分比 11px）

## 1.9.28

- 缩放百分比文字 12px → 11px，与 10.5px 的图标等高更协调
- 同时把标签盒子 30px → 34px：`#zoom-label` 没有设 `overflow`，11px 下四位数字实测字形宽 31.2px，超出 30px 盒子后会向两侧溢出、压到 `−` 和 `+` 图标上（看起来挤成一团）。加宽后 `30%`/`100%`/`140%`/`200%` 全部完整落在盒子内，数字左右各留 1.4px；按钮盒子与图标尺寸不变

## 1.9.27

- 新增「独立 / 统一侧栏」开关（设置页 → 侧边栏行为，`deepseek-sidebar-per-tab-sidebar`，默认独立，修改立即生效并自动保存）
- 关闭开关后进入统一侧栏模式：所有标签页共享同一份侧栏状态（选中的应用、缩放、Harness 会话和各 AI 的对话路由），新标签页延续当前内容，切换标签页时侧栏保持不变；关闭标签页不再清除这份共享状态
- 切换开关时无缝迁移当前侧栏内容：改为统一时把当前标签页的状态并入共享槽，改回独立时把共享状态还给当前标签页，其余标签页保留各自原有的状态
- 实现上引入固定 `shared` 状态槽（`tab-state.js` / `sidepanel-context.js` 新增支持），统一模式下侧栏文档重新显示时会从共享槽重新同步，隐藏期间被其它标签页改过的内容也能及时跟上

## 1.9.26

- 新安装默认不显示 **DeepSeek Harness** 和 **有道词典**：默认值收敛到 `app-catalog.js` 的 `defaultHidden` 字段（`visibleByDefault()`），侧边栏与设置页共用同一条规则；在设置页勾选后照旧显示（用户显式选择的优先级高于默认值）
- 新标签页的默认应用不再落到被隐藏的应用上：没有偏好记录时取 `firstVisibleApp()`，因此新装侧栏打开的是 DeepSeek 而不是看不见的 Harness 条目
- 设置页「恢复默认」现在恢复的是出厂默认（只显示六个聊天站点），而不是把全部应用都打开；设置页副标题也标注了默认隐藏的两项

## 1.9.25

- 右上角操作按钮整体缩小一半：按钮盒子 20px → 10px、图标 13px → 7px、缩放手势标签字号 10px → 8px（宽度 28px → 20px）、圆角 4px → 2px；`stroke-width` 由 2.2 提到 4（= 2.2 × 13/7），缩小后每条线在小尺寸下仍然是同样的视觉粗细，六个图标依旧共用同一个 `viewBox` 和同一条 CSS 规则
- 左侧应用图标尺寸不变，主次关系更清楚

## 1.9.24

- 修复侧栏会「自己消失」：Chrome 给每个标签页都保留一份侧栏文档，隐藏文档里的 iframe（Harness 页面、AI 站点及它们的连接）会一直存活，访问的标签页越多占用越大；一旦内存压力杀掉侧栏渲染进程，Chrome 会**直接移除侧栏且不显示任何错误页**。现在侧栏隐藏超过 30 秒（`PANEL_IDLE_PARK_MS`）会释放该文档的全部 iframe，切回该标签页时自动重建（实测：4 个标签页从 10 个 iframe / 2.8GB 降到 1 个 iframe / 2.0GB）
- 选择页面元素时限制送往侧栏的数据：文本最多 20 万字符（超出会在阅读器标注「已截断」），不再把整个元素的 HTML 传进侧栏，只报告长度；没有可见文本的元素保留 2000 字符的 HTML 预览

## 1.9.23

- 工具栏图标统一为同一套内联 SVG（`--toolbar-icon-size: 13px`、同样的描边、线帽与颜色），不再依赖 `⌖ ↻ ⚙` 等系统字体字形，不同平台下大小和粗细一致；右侧操作图标也比之前更小更紧凑
- 修复选择页面元素后无法填充到输入框：重新由 `page-bridge.js` 把选中的文本写入当前侧栏应用（Harness 对话页或 AI 站点）的输入框，并兼容 Lexical 等 contenteditable 编辑器
- 阅读器状态会显示填充结果（已填充 / 未找到可填充的输入框 / 页面未加载）

## 1.9.22

- 修复 DSH 开启浏览器认证后侧栏只显示 `dsh web authentication required` 的问题
- 端口探测改为优先选择可直接嵌入的 DSH 页面（例如 DeepSeek Harness Desk 提供的免认证实例），再复用它的 bridge 地址

## 1.9.21

- 侧栏 iframe 底色跟随浏览器/系统深浅色主题，深色模式下切换顶部应用或浏览器标签不再闪白屏
- 多 AI 对比工作台的 iframe 底色也改为深色，AI 页面加载瞬间不再出现白色闪屏

## 1.9.20

- 收紧侧栏顶部右侧操作区，并将“多 AI 对比”按钮改为三栏竖向图标

## 1.9.19

- 重新设计多 AI 对比页面按钮，统一顶部操作、AI 选择、发送和结果栏操作的视觉与交互状态

## 1.9.18

- 进一步收紧顶部工具栏的按钮与图标尺寸，在窄侧栏中保持应用图标和操作按钮的统一落点

## 1.9.17

- 顶部工具栏按钮改为更紧凑的固定尺寸，应用图标不再被挤压，右侧操作按钮保持统一位置

## 1.9.16

- 修复侧栏保持打开时新建标签页选中的 AI 图标回到 Harness 的问题
- 新标签页继承最近一次选中的 AI，同时保留已有标签页各自的应用和缩放状态

## 1.9.15

- 修复侧栏保持打开时新建标签页缩放回到 100% 的问题
- 新标签页继承最近一次设置的缩放比例，同时保留已有标签页各自的缩放状态

## 1.9.14

- 多 AI 对比工作台增加 30%–200% 缩放、快捷键和双击恢复 100%
- 自动保存多 AI 工作台缩放比例，重新打开或刷新页面后恢复

## 1.9.13

- 新增全屏“多 AI 对比”工作台，可自由选择 DeepSeek、智谱、千问、Kimi、ChatGPT、Gemini
- 统一输入一次问题，并行投递到所有选中 AI；各栏直接显示原站返回结果和投递状态
- 支持记住所选 AI、单栏/全部刷新、打开原站、浏览器全屏，以及 Ctrl/⌘ + Enter 快捷发送
- 抽取共享 `app-catalog.js`，以后新增或更新 AI 站点时侧栏、设置页和多 AI 页面同步生效
